...
fn main() {
    correctness_matrix();
    f1_router_projection();
    f1_router_projection_offset();
...
/// Matches the dense router projection in F1 prefill: a contiguous token
/// activation multiplied by a checkpoint weight exposed as a transposed view.
/// The small generic matrix below covers transpose semantics, while this shape
/// also exercises the cuBLASLt heuristic selected for the real workload.
fn f1_router_projection() {
    f1_router_projection_layout(true);
}

/// Exercises the same shape through an offset view, which requires a distinct
/// cuBLASLt alignment-aware plan from the aligned F1 tensor above.
fn f1_router_projection_offset() {
    f1_router_projection_layout(false);
}

fn f1_router_projection_layout(exact_layout: bool) {
    check(Problem {
        m: 208,
        n: 256,
        k: 2048,
...
fn check(problem: Problem) {
    ...
    let rhs_padding = problem.padding + usize::from(!problem.exact_layout) * 2;
    let rhs_batch_gap = problem.batch_gap + usize::from(!problem.exact_layout) * 3;
    let rhs_offset = problem.offset + usize::from(!problem.exact_layout) * 5;
    let out_padding = problem.padding + usize::from(!problem.exact_layout) * 4;
    let out_batch_gap = problem.batch_gap + usize::from(!problem.exact_layout) * 7;
    let out_offset = problem.offset + usize::from(!problem.exact_layout) * 9;
    let rhs = Matrix::new(
        problem.k,
        problem.n,

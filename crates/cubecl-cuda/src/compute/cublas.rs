...
impl PlanKey {
    fn new(
        ...
        let matrix = |m: &GemmMatrix, resource: &GpuResource| MatrixKey {
            leading_dimension: m.leading_dimension,
            batch_stride: m.batch_stride,
            transposed: m.transposed,
            alignment: pointer_alignment(resource.ptr),
        };
        Self {
            m: descriptor.m,
            n: descriptor.n,
            ...
            lhs: matrix(&descriptor.lhs, lhs),
            rhs: matrix(&descriptor.rhs, rhs),
            out: matrix(&descriptor.out, out),
        }
    }
}
...
impl CublasState {
    pub(crate) fn launch(
        ...
        let key = PlanKey::new(descriptor, lhs, rhs, out);
        if !self.plans.contains_key(&key) {
            let plan = build_plan(handle, descriptor, lhs, rhs, out)?;
            self.plans
                .insert(PlanKey::new(descriptor, lhs, rhs, out), plan);
        }
        let plan = self
            .plans
            .get(&key)
...
fn build_plan(
    ...
    // The selected algorithm must be valid for the actual views, not merely
    // their shape and strides. A CubeCL binding can start at an arbitrary byte
    // offset, and cuBLASLt algorithms have distinct A/B/C/D alignment
    // contracts. Constrain the heuristic with each resolved pointer's actual
    // alignment; the plan key carries the same values so it is never reused
    // for a less-aligned view.
    for (attr, alignment) in [
        (
            sys::cublasLtMatmulPreferenceAttributes_t::CUBLASLT_MATMUL_PREF_MIN_ALIGNMENT_A_BYTES,
            pointer_alignment(rhs.ptr),
        ),
        (
            sys::cublasLtMatmulPreferenceAttributes_t::CUBLASLT_MATMUL_PREF_MIN_ALIGNMENT_B_BYTES,
            pointer_alignment(lhs.ptr),
        ),
        (
            sys::cublasLtMatmulPreferenceAttributes_t::CUBLASLT_MATMUL_PREF_MIN_ALIGNMENT_C_BYTES,
            pointer_alignment(out.ptr),
        ),
        (
        ...
        // SAFETY: `pref` is live and every alignment preference is a u32.
        if let Err(err) = unsafe {
            lt::set_matmul_pref_attribute(
                pref,
                attr,
                (&alignment as *const u32).cast::<c_void>(),
...
fn pointer_alignment(ptr: u64) -> u32 {
    // cuBLASLt accepts power-of-two alignment requirements. A non-null CUDA
    // resource has at least byte alignment; cap at 256 because larger values
    // do not further distinguish the algorithms used here.
    1u32 << ptr.trailing_zeros().min(8)
}
